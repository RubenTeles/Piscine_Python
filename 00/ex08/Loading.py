...
# yield, a função é um gerador, e o tipo de retorno
# correto é Generator, não None.
...
# [{elapsed_formatted}<{eta_formatted}, {speed:.2f}it/s]"
...
def ft_tqdm(lst: range) -> None:
    """
    A custom implementation of the tqdm progress bar.

    Iterates over a given range while displaying a dynamic progress bar
    in the terminal. The bar shows the current percentage completed,
    a visual bar of fixed length, and the current step versus the total.

    Args:
        lst (range): The range (or iterable with length) to iterate over.

    Yields:
        item: Each element from the input range, one by one.
    """
    total = len(lst)
    bar_length = 100
    total_digits = len(str(total))
